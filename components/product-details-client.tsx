...
export default function ProductDetailsClient({ product }: { product: any }) {
  ...
  const handleOrderSingleViaWhatsApp = () => {
    const text = `Hello Noor Store, I would like to order: ${product.name} (Price: EGP ${product.price.toFixed(2)})`
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      <div className="flex items-center space-x-2 text-xs text-brand-muted mb-8">
        <button onClick={() => router.push('/')} className="hover:text-brand-600 flex items-center gap-1">
          <span className="material-symbols-outlined text-[14px]">arrow_back</span> Back to Collection
        </button>
        <span>/</span>
        <span>{product.category || 'Beauty'}</span>
        <span>/</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          ...
          <div className="lg:col-span-6 flex flex-col gap-4">
            <div className="relative rounded-2xl overflow-hidden bg-brand-cream border border-brand-200 aspect-square shadow-inner">
              ...
              />
              <div className="absolute top-4 left-4 bg-white/90 backdrop-blur-sm px-3 py-1 rounded-full text-xs font-semibold text-brand-600 shadow-sm">
                {product.category || 'Luxury Formula'}
              </div>
            </div>
            
            <div className="grid grid-cols-3 gap-3">
              <div className="border-2 border-brand-600 rounded-xl overflow-hidden aspect-video cursor-pointer">
                <img src={product.image_url || 'https://images.unsplash.com/photo-1522337360788-8b13dee7a37e?auto=format&fit=crop&w=800&q=80'} alt="Thumbnail 1" className="w-full h-full object-cover" />
              </div>
              <div className="border border-brand-200 rounded-xl overflow-hidden aspect-video bg-brand-cream flex items-center justify-center text-xs text-brand-muted opacity-75">
                <span>Texture Macro</span>
              </div>
              <div className="border border-brand-200 rounded-xl overflow-hidden aspect-video bg-brand-cream flex items-center justify-center text-xs text-brand-muted opacity-75">
                <span>Model Swatch</span>
              </div>
            </div>
          </div>
          ...
          <div className="lg:col-span-6 flex flex-col justify-between">
            <div>
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-xs text-brand-muted">(140 verified boutique reviews)</span>
              </div>
              <h1 className="font-serif text-3xl sm:text-4xl lg:text-5xl font-bold text-brand-dark leading-tight mb-4">
                {product.name}
              </h1>
              <p className="font-serif text-2xl text-brand-600 mb-6">EGP {Number(product.price).toFixed(2)}</p>
              
              <div className="prose prose-sm text-brand-muted">
                <p className="leading-relaxed">
                  {product.description || 'Experience the essence of luxury. Handcrafted with botanical ingredients to nourish and elevate.'}
                </p>
              </div>
            </div>
            ...
            <div className="mt-10 pt-8 border-t border-brand-200">
              <div className="flex flex-col sm:flex-row gap-4 items-end sm:items-center">
                <div className="w-full sm:w-auto">
                  <span className="text-[10px] uppercase tracking-widest text-brand-muted mb-2 block">Quantity</span>
              ...
              <div className="mt-4">
                ...
                >
                  ...
                  {product.stock === 0 ? 'Unavailable via WhatsApp' : 'Fast Order via WhatsApp'}
                </button>
              </div>
